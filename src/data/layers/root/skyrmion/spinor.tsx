...
const layer = createLayer(id, function (this: BaseLayer) {
    ...
    const productionModifiers = createSequentialModifier(() => [
        ...
        createMultiplicativeModifier(() => ({
            ...
            description: () => <>[{skyrmion.name}] {unref(skyrmion.pion.pions.singularName)} Upgrade γ ({format(unref(pion.upgrades.gamma.totalAmount))})</>
        })),
        createMultiplicativeModifier(() => ({
            multiplier: pion.upgrades.mu.effect,
            enabled: () => Decimal.gt(unref(pion.upgrades.mu.totalAmount), 0),
            description: () => <>[{skyrmion.name}] {unref(skyrmion.pion.pions.singularName)} Upgrade μ ({format(unref(pion.upgrades.mu.totalAmount))})</>
        }))
    ]);
    ...
    const costNerf = computed(() => {
        const amount = unref(inAbyss) ? unref(abyss.upgradeCount) : unref(pion.upgradeCount);
    ...
    const nextCostNerf = computed(() => {
        const amount = unref(inAbyss) ? unref(abyss.nextUpgradeCount) : unref(pion.upgradeCount).plus(1);
    ...
    const modifierModal = createModifierModal(
        () => `${unref(spinors.singularName)} Modifiers`,
        () => [{
