...
const layer = createLayer(id, function (this: BaseLayer) {
    ...
    const pions = createResource<DecimalSource>(0, name);
    const productionModifiers = createSequentialModifier(() => [
        ...skyrmion.production,
        createMultiplicativeModifier(() => ({
            multiplier: spinor.upgrades.gamma.effect as Computable<DecimalSource>,
            description: jsx(() => (<>Spinor Upgrade γ ({0})</>))
        })),
        createMultiplicativeModifier(() => ({
            multiplier: spinor.upgrades.mu.effect as Computable<DecimalSource>,
    ...
    this.on("preUpdate", diff => {
        ...
        pions.value = acceleratedDiff.times(production.evaluate()).plus(pions.value);
    })
    ...
    const upgrades: Record<string, SkyrmionRepeatable> = {
        ...
        epsilon: createUpgrade(() => ({
            ...
            effect: amount => Decimal.max(unref(fome.amounts[FomeTypes.infinitesimal]), 0).plus(1).log10().times(amount).times(0.5).plus(1),
        ...
        theta: createUpgrade(() => ({
            ...
            effect: amount => Decimal.max(unref(fome.amounts[FomeTypes.subspatial]), 0).plus(1).log10().times(amount).plus(1),
        ...
        kappa: createUpgrade(() => ({
            ...
            effect: amount => Decimal.add(unref(fome.boosts[FomeTypes.protoversal][1].amount), unref(fome.boosts[FomeTypes.protoversal][1].bonus)).times(amount).times(0.3).plus(1),
            bonusAmount: fome.boosts[FomeTypes.quantum][4].effect
        })),
